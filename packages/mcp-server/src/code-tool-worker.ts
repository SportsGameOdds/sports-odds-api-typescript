...
import { SportsGameOdds, ClientOptions } from 'sports-odds-api';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: SportsGameOdds)`
    : `const run: (${functionSource.client}: SportsGameOdds) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
