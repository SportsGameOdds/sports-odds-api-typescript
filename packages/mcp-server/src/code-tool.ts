...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    SPORTS_ODDS_API_KEY_HEADER: readEnv('SPORTS_ODDS_API_KEY_HEADER') ?? client.apiKeyHeader ?? undefined,
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = 'sports-odds-api';
  ...
  try {
    const resp = await new Promise<Response>((resolve, reject) => {
      ...
      const opts = {
        ...
        ...(client.apiKeyHeader != null ? { apiKeyHeader: client.apiKeyHeader } : undefined),
        ...(client.apiKeyParam != null ? { apiKeyParam: client.apiKeyParam } : undefined),
        defaultHeaders: {
